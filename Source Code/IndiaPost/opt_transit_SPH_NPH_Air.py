...
def opt_transit_nph_air(source_id, dest_id, dispatch_datetime,buff_dept, buff_arrival):
	...
	if(source_id != source_id_proxy):
		...
		list_transit_modes.append('0')
		arr_datetime_wb, dept_datetime, arr_datetime = dept_road(dispatch_datetime, float(list_surf_distance[source_id][source_id_proxy]), buff_dept, buffer_tp_surf)
		...
		buffer_origin_surf_pr = buff_dept
		buffer_origin_air_pr = buff_dept

	if(dest_id != dest_id_proxy):
		buffer_dest_surf_pr = buffer_tp_surf
		buffer_dest_air_pr = buffer_tp_air
	else:
		buffer_dest_surf_pr = buff_arrival
		buffer_dest_air_pr = buff_arrival
	...
	if(dest_id_proxy != dest_id):
		...
		list_transit_modes.append('0')
		arr_datetime_wb, dept_datetime, arr_datetime = dept_road(list_opt_path[0], float(list_surf_distance[dest_id_proxy][dest_id]), buffer_tp_surf, buff_arrival)
		list_transit_timestamps.extend([dept_datetime, arr_datetime, arr_datetime_wb])
...
def main():
	...
	fp_in = openpyxl.load_workbook(curDir +'SPH to NPH.xlsx')
	...
	if(num_rows>0):
		...
		for i in range(num_rows):
			...
			dest_NPH_id = dict_loc_to_id[destination_name_NPH]
			...
			if(source_NPH_id ==dest_NPH_id):
				...
				list_transit_locations, list_transit_modes, list_transit_timestamps, arrival_datetime = opt_transit_nph_all(source_SPH_id, source_NPH_id, dispatch_datetime,buffer_origin_surf, buffer_nph_arr)
				list_transit_locations_air, list_transit_modes_air, list_transit_timestamps_air, arrival_datetime_air = opt_transit_nph_air(source_SPH_id, source_NPH_id, dispatch_datetime,buffer_origin_air, buffer_nph_arr)
				...
				list_transit_locations, list_transit_modes, list_transit_timestamps, arrival_datetime = opt_transit_nph_all(source_NPH_id, dest_NPH_id, opt_overall_1[0],buffer_nph_dep, buffer_nph_arr)
				list_transit_locations_air, list_transit_modes_air, list_transit_timestamps_air, arrival_datetime_air = opt_transit_nph_air(source_NPH_id, dest_NPH_id, opt_overall_1[0],buffer_nph_dep, buffer_nph_arr)
				...
				opt_overall[2] = opt_overall_1[2]+ [source_NPH_id] + opt_overall_2[2]  
			...
			sheet_out.cell(row = i+2, column = 3).value = destination_name_NPH
			...
			curr_col += 1
			sheet_out.cell(row = i+2, column = curr_col).value = destination_name_NPH
			curr_col += 1
