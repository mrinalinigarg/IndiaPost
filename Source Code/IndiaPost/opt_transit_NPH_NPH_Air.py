...
def closest_arrival(dept_datetime, list_mode, buff_dept, buff_arrival):
	...
	for mode_id in range(total_index):
		...
		print(dept_time, transit_duration)
...
def opt_transit_nph_all(source_id, dest_id, dispatch_datetime):
	...
	if(no_tp_rows == 0):
		...
		for i in range(no_tp_rows):
			...
			datetime_arr_road_wb, datetime_dept_road, datetime_arr_road = dept_road(dispatch_datetime, float(list_surf_distance[source_id][list_transit[0]]), buffer_origin_surf, buffer_tp_surf)
			...
			datetime_arr_rail_wb, datetime_dept_rail, datetime_arr_rail = closest_arrival(dispatch_datetime, list_rail[index_leg], buffer_origin_surf, buffer_tp_surf)					
			...
			datetime_arr_road_wb, datetime_dept_road, datetime_arr_road = dept_road(datetime_arrival, float(list_surf_distance[list_transit[length_transit-1]][dest_id]), buffer_tp_surf, buffer_dest_surf)
			#datetime_arr_air_wb, datetime_dept_air, datetime_arr_air = closest_arrival(dispatch_datetime, list_air[index_leg], buffer_origin_air_pr, buffer_tp_air)
			datetime_arr_rail_wb, datetime_dept_rail, datetime_arr_rail = closest_arrival(datetime_arrival, list_rail[index_leg], buffer_tp_surf, buffer_dest_surf)					
			list_mode_2 = min([[datetime_arr_road_wb, datetime_dept_road, datetime_arr_road, '0'], [datetime_arr_rail_wb, datetime_dept_rail, datetime_arr_rail, '2']])					
			datetime_arrival = list_mode_2[0]
			list_timestamps_tp.extend([list_mode_2[1], list_mode_2[2], list_mode_2[0]])
			list_mode_tp.append(list_mode_2[3])	
...
def main():
	...
	fp_in = openpyxl.load_workbook(curDir +'NPH to NPH.xlsx')
	...
	if(num_rows>0):
		...
		for i in range(num_rows):
			...
			list_transit_locations, list_transit_modes, list_transit_timestamps, arrival_datetime = opt_transit_nph_all(source_id, dest_id, dispatch_datetime)
			list_transit_locations_air, list_transit_modes_air, list_transit_timestamps_air, arrival_datetime_air = opt_transit_nph_air(source_id, dest_id, dispatch_datetime)
			air_arrival_datetime, air_datetime_dept, air_datetime_arr = closest_arrival(dispatch_datetime, list_air[source_id*num_locations + dest_id], buffer_origin_air, buffer_dest_air)
			rail_arrival_datetime, rail_datetime_dept, rail_datetime_arr = closest_arrival(dispatch_datetime, list_rail[source_id*num_locations + dest_id], buffer_origin_surf, buffer_dest_surf)
			road_arrival_datetime, road_datetime_dept, road_datetime_arr = dept_road(dispatch_datetime, float(list_surf_distance[source_id][dest_id]), buffer_origin_surf, buffer_dest_surf)
			...
			sheet_out.cell(row = i+2, column = 2).value = source_name
			sheet_out.cell(row = i+2, column = 3).value = dest_name
			...
			sheet_out.cell(row = i+2, column = 7).value = opt_overall[1][0].time()
			print(source_name,dest_name ,opt_overall[1][0])
			curr_col += 1
			sheet_out.cell(row = i+2, column = 8).value = list_days_week[opt_overall[1][0].weekday()]#weekday
			curr_col += 1
			sheet_out.cell(row = i+2, column = 9).value = opt_overall[1][1].time()
			curr_col += 1
			sheet_out.cell(row = i+2, column = 10).value = list_days_week[opt_overall[1][1].weekday()]
			curr_col += 1
			sheet_out.cell(row = i+2, column = 11).value = opt_overall[1][2].time()
			curr_col += 1
			sheet_out.cell(row = i+2, column = 12).value = list_days_week[opt_overall[1][2].weekday()]
			curr_col += 1
			...
			curr_col += 1
			sheet_out.cell(row = i+2, column = curr_col).value = dest_name


			time_diff = timedelta.total_seconds((opt_overall[0]) - (dispatch_datetime))

			print(time_diff/3600)
			sheet_out.cell(row = i+2, column = 6).value = (time_diff/3600)
