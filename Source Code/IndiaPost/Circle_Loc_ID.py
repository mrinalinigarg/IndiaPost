...
data_list[1].append(5)
data_list[1].append(4)
data_list[1].append(2)
data_list[1].append(3)
data_list[1].append(15)
data_list[1].append(61)
loc_circle=1
...
for i in range(num_loc):
	...
	location_id = int(loc_id_list[i][0]) - 1
	circle_name = str(loc_id_list[i][4])
	circle_id = int(loc_id_list[i][7])
	
	data_list[circle_id+1].append(location_id)
...
with open('C:\\web2py_win\\web2py\\applications\\IndiaPost_UI\\Source Code\\IndiaPost\\Circle_Loc_ID.csv', 'w', newline = '') as fp_out: #Writing the values as list of lists in 1 go to decrease time complexity
	writer = csv.writer(fp_out)
	writer.writerows(data_list)
wb = Workbook()
ws = wb.active
with open('C:\\web2py_win\\web2py\\applications\\IndiaPost_UI\\Source Code\\IndiaPost\\Circle_Loc_ID.csv', 'r') as f:
    for row in csv.reader(f):
        ws.append(row)
wb.save('C:\\web2py_win\\web2py\\applications\\IndiaPost_UI\\Source Code\\IndiaPost\\Circle_Loc_ID.xlsx')