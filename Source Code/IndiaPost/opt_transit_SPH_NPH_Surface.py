	...
	if(num_rows>0):
		...
		for i in range(num_rows):
			...
			for j,k,l in zip(range(3,num_timestamps+2,3),range(num_tp),range(num_modes)):
				print(num_timestamps,num_tp,num_modes)
				...
				curr_col += 1
				k+=1
				print(i,j,num_timestamps)
				print(opt_overall[1])
				print(opt_overall[1][j],opt_overall[1][j+1],opt_overall[1][j+2])
			...
			#sheet_out.cell(row = i+2, column = 72).value = 

			fp_out.save(curDir +'Surface_SPH to NPH.xlsx')
			#fp_out.save('C:\\web2py_win\\web2py\\applications\\IndiaPost\\private\\Output.xlsx')


			#fp_out.save('SPH_NPH_Surface_Output_12June.xlsx')
